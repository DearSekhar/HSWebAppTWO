...
export GKE_APP_NAME=hschan-demo-webapp1
export GKE_SERVICE=hschan-service
export GKE_SERVICE_ACCOUNT=hschan-serviceaccount
export GKE_DEPLOYMENT_NAME=hschan-demo-webapp1-deployment
...
# Create deployment
envsubst < HSWebAppTWO/deployment.yml | kubectl apply -f -

# Create service
envsubst < HSWebAppTWO/service.yml | kubectl apply -f -
...
#git clone https://github.com/dearsekhar/HSWebAppTWO.git

#docker build --tag "$GKE_REGION-docker.pkg.dev/$GKE_PROJECT/$GKE_PROJECT/$GKE_APP_NAME:$GITHUB_SHA" HSWebAppTWO/

#us-central1-docker.pkg.dev/sincere-actor-374621/sincere-actor-374621/hschan-demo-webapp1:Test
#docker build --tag "$GKE_REGION-docker.pkg.dev/$GKE_PROJECT/$GKE_PROJECT/$GKE_APP_NAME:$GITHUB_SHA" HSWebAppTWO/HSWebAppTWO/
